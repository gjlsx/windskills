...
All taskexec path fields are scope-relative paths inside `taskexec/windskills/`.

## Fixed Placement Rules

Within `taskexec/windskills/`:
...
1. After review passes and before doing task execution, register the tasklist in `taskexec/windskills/tasklistall.md` with `Dir` set to `codex/` according to the repository `.agent-rules.md`.
...
`tasklistall.md` is only an index of tasklist files for `taskexec/windskills/`.
