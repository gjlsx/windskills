...
- `tasklistall.md`
- `tasklistMMDDhhmm.md` at the scope-root top level for active tasklists
...
Do not place active tasklists under a custom subdirectory. Do not invent another tasklist directory. Do not search the whole repository for tasklists as a fallback.
...
- active `tasklistMMDDhhmm.md` files directly under `taskexec/<subproject>/`
- `taskexec/<subproject>/docs/backuptask/`
- `taskexec/<subproject>/docs/qa/`
- `taskexec/<subproject>/docs/issue_index.md`
- `taskexec/<subproject>/docs/issues/`
...
4. The target tasklist file named by the user, but only if it is at the scope-root top level or under `docs/backuptask/`; otherwise treat it as invalid
...
- Inside that scope root, create `tasklistall.md`, `docs/backuptask/`, `docs/qa/`, `docs/issue_index.md`, `docs/issues/`, `locks/`, `tasklog/`, and `details/`.
...
- Create the first `tasklistMMDDhhmm.md` at the scope-root top level when the first tasklist is needed.
...
7. If the workflow uses direct-fix issues outside taskexec task rows, store the issue index under `docs/issue_index.md` and each issue detail under `docs/issues/` inside the same scope root.
8. For a same-workflow follow-up bugfix, add a new `bugfix` row to the same active tasklist instead of rewriting the completed row. Include `FIX:<source_taskid>` in the row `Description` or referenced `Detail`.
9. Only the tasklist creator may append follow-up tasks or create/edit their `Detail` files. If the creator is unavailable, stop and hand the decision back to a human.
10. Run the required self-review before claiming the tasklist is ready.
11. Create the tasklist at the scope-root top level.
12. Register the tasklist in the scope-local `tasklistall.md` with `Dir` set to `./`.
13. After registration, read that same `tasklistall.md` and run the repository's active-tasklist sweep rule only against the scope-root top level and `docs/backuptask/` inside the same scope root. Update the same `tasklistall.md` and commit the resulting change.
14. Commit the tasklist build/update with the repository's tasklist commit rule before moving on.
...
2. If the user gave a tasklist path, it must be at the scope-root top level or under `docs/backuptask/` inside the resolved scope root. Otherwise stop and report an invalid tasklist location.
...
If the user asks to archive/submit, or local rules require an archive sweep before creating the next tasklist, follow the repository's archive sweep rule after the tasklist state commit. Archive sweeps are scope-local and only touch the scope-root top level and `docs/backuptask/`.
...
- direct-fix issue index/details under `docs/issue_index.md` and `docs/issues/` when the repository uses the issue workflow
...
- Active tasklists live only at the scope-root top level.
...
- Build tasklist: read root rules -> resolve scope root -> create `details/` and `docs/qa/` files if needed -> create tasklist at scope-root top level -> self-review -> register in scope-local `tasklistall.md` with `Dir=./` -> sweep scope-root top level/`docs/backuptask/` -> commit
...
- Follow-up bugfix: reuse the same active non-archived tasklist at the scope-root top level when scope still matches
...
- Placing active tasklists under a custom subdirectory instead of directly under `taskexec/<subproject>/`
- Inventing another tasklist directory besides the scope-root top level and `docs/backuptask/`
...
- `Use taskexec to run taskexec/windskills/tasklist04210212.md`
- `Use taskexec to append a follow-up bugfix to taskexec/shop-admin/tasklist04230500.md`
...
4. Inside that scope root, create `tasklistall.md`, `docs/backuptask/`, `docs/qa/`, `docs/issue_index.md`, `docs/issues/`, `locks/`, `tasklog/`, and `details/`.
...
7. Create the first `tasklistMMDDhhmm.md` at the scope-root top level when work begins.
8. Copy `scripts/new_issue.sh` and `scripts/gen_issues.sh` to the repository `/scripts/` dir if the repository wants the bundled issue workflow, then invoke them with the scope root, for example `scripts/new_issue.sh taskexec/<subproject>`.
